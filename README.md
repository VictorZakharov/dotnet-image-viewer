...
- Right-click an image and use **Edit** to rotate, resize, crop, convert, watermark, or remove metadata; the same commands are directly available in the full-image viewer
...
Right-click an image in the browser and open **Edit**, or right-click anywhere
in the full-image viewer, to rotate left/right, resize, crop, convert to JPEG,
PNG, WebP, or TIFF, add a text watermark, or remove metadata. Each focused
dialog validates its dimensions, format, filename, and operation support before
enabling the action.
